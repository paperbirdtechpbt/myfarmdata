                    ...
                    <div class="panel-body">
                        <div class="row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-9">
                                            ...
                                            <select class="form-control select" name="community_group_id" id="community_group_id" value="{{old('name')}}">
                                                <!--<option value="">select</option>-->
                                                @foreach($communitygrp as $community_group)
                                                <option value="{{$community_group->id}}" @if($community_group->id == $lists->community_group->id) selected @endif>{{$community_group->name}}</option>
                                                @endforeach
                                            </select>
                                            <span class="help-block">This field is required</span>
                                        </div>
                            ...
                            <div class="col-md-12" id="privilege_div" style="margin-top: 20px;">
                                ...
                                <div class="col-md-6" id="priv_div{{$loop->iteration}}">
                                    <div class="form-group">
                                        <label class="col-md-3 control-label">Choice</label>
                                        <div class="col-md-9">
                                            <div class="input-group">
                                                <input type="text" class="form-control" name="choice{{$loop->iteration}}" id="choice{{$loop->iteration}}" value="{{$choice->name}}">
                                                <span class="input-group-btn">
                                                    <button class="btn btn-default" type="button" onclick="delete_row({{$loop->iteration}})"><i class="fa fa-trash-o" style="color: red;"></i></button>
                                                </span>
                                            </div>
                                            <span class="help-block">This field is required</span>
                                            <input type="hidden" name="privilege_id{{$loop->iteration}}" id="privilege_id{{$loop->iteration}}" value="{{$choice->id}}">
                                            <input type="hidden" name="privilege_is_delete{{$loop->iteration}}" id="privilege_is_delete{{$loop->iteration}}" value="0">
                                        </div>
                                    </div>
                                </div>
                                @endforeach
